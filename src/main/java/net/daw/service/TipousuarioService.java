...
public class TipousuarioService {
	...
	public ReplyBean create() throws Exception {
		ReplyBean oReplyBean;// Es un puto mensaje para averiguar si la operacion ha salido bien o mal en la
								// base de datos.
		...
		try {
			...
			oConnectionPool = ConnectionFactory.getConnection(ConnectionConstants.connectionPool);
			oConnection = oConnectionPool.newConnection();
			TipousuarioDao oTipousuarioDao = new TipousuarioDao(oConnection, ob);// la conexion siempre viaja a traves
																					// de la petición para no saturar el
	...
	public ReplyBean crearDatos() throws Exception {
		...
		ConnectionInterface oConnectionPool = null;
		Connection oConnection;
		String[] desc = { "Cliente", "Administrador", "Becario", "Pintamonas" };
		...
		try {
			...
			throw new Exception("ERROR: No entra bien en el DAO. ", ex);
		} finally {
			oConnectionPool.disposeConnection();
		}
		return oReplyBean;
